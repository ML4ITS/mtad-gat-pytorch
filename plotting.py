...
class Plotter:
    ...
    def create_shapes(self, ranges, sequence_type, _min, _max, plot_values, is_test=True, xref=None, yref=None):
        ...
        for r in ranges:
            ...
            if self.use_timestamps:
                ...
                x1 = self.test_timestamps[r[1] + w] if is_test else self.train_timestamps[r[1] + w]
            else:
                x0 = r[0] - w
                x1 = r[1] + w
            shape = {
    ...
    def plot_global_predictions(self, type="test"):
        ...
        axs[0].plot(self.train_output[f"A_Score_Global"], c="r", label="train anomaly scores")
        axs[0].plot(data_copy["Thresh_Global"], linestyle="dashed", c="black")
        axs[1].plot(data_copy[f"A_Score_Global"], c="r", label="anomaly scores")
        axs[1].plot(data_copy["Thresh_Global"], linestyle="dashed", c="black", label="threshold")
        axs[2].plot(data_copy["A_Pred_Global"], label="predicted anomalies", c="orange")
        if self.labels_available and type == "test":
            axs[3].plot(data_copy["A_True_Global"], label="actual anomalies",)
        axs[0].set_ylim([None, 2 * data_copy["Thresh_Global"].mean()])
        axs[1].set_ylim([None, 2 * data_copy["Thresh_Global"].mean()])
    ...
    def plotly_global_predictions(self, type="test"):
        ...
        true_anomaly_sequences =self.get_anomaly_sequences(data_copy[f"A_True_Global"].values) if self.labels_available else None
        y_min = -0.1
        y_max = 1.1 * np.max(tot_anomaly_scores)  # 0.5 * y_max
        ...
        layout = {
            ...
            'height': 400,
            'width': 1100,
            'xaxis_range': [self.start_datetime if is_test else self.test_start,
                            self.train_end if not is_test else self.end_datetime] if self.use_timestamps else None
            # 'template': 'simple_white'
        }

        fig = go.Figure(data=[go.Scatter(x=None, y=tot_anomaly_scores, line=dict(width=1, color='red'))], layout=layout)
        py.offline.iplot(fig)